# Generate SINGLE-PANEL synthetic astronomical figures of type "contour" and
# "image of the sky".
#
# Same generator, same prechecks and same accept/reject checks as the top-level
# ../create_figures_batch.py -- the only difference is that the panel count is
# pinned to 1 rather than sampled, so -panel_min / -panel_median / -panel_max
# are not offered here.
#
# Why this exists: multipanel figures are not currently generatable.  The two
# tick prechecks brought box-overlap rejections down from 67% of attempts to 9%,
# but `title axis off page` still rejects essentially every multipanel attempt
# (~900 occurrences in 639 attempts), so a multipanel figure costs hours where a
# single-panel one costs about a minute.  This directory is the reliable path
# while that is unresolved; the top-level script keeps the full behaviour.
#
# run like:
#   python single/create_figures_batch.py -save_dir ./synthetic_figures/ -number_of_figures 20
#   mpirun -np 6 python single/create_figures_batch.py -save_dir ./synthetic_figures/ -number_of_figures 700
#
